		...
		<view class="wanl-product">
						...
						<view class="warter left" v-for="(item, index) in leftList" :key="index" @tap="onGoods(item.id)">
							...
							<view class="content padding-bj">
								<view class="text-cut-2">
									<block v-if="item.shop.isself!==undefined && item.shop.isself == 1">
									...
									<text v-if="item.title!==undefined">{{ item.title }}</text>
								</view>
								<view class="goods-tag">
									<view class="cu-tag radius sm line-red"  v-if="item.shop.isself!==undefined && item.shop.isself == 1">官方放心购</view>
									<view class="cu-tag radius sm line-gray" v-if="item.isLive" @tap="onLive(item.isLive)">LIVE</view>
								</view>
								<view class="flex align-center justify-between">
									<view class="text-red text-bold text-lg" v-if="item.price!==undefined">
										<text class="text-price">{{ item.price }}</text>
									</view>
									<view class="text-sm wanl-gray" v-if="item.comment!==undefined">
			...
			<view class="product_list" :class="dataStyle" v-else>
				<view class="item" v-for="(item, index) in dataList" :key="index" @tap="onGoods(item.id)">
					<view class="img-wrap"><image :src="$wanlshop.oss(item.image, 125, 125)" mode="aspectFill"></image></view>
					<view class="content padding-sm">
						...
						<view class="flex align-center justify-between" v-if="item.price!==undefined">
			...
			<!-- 兼容字节跳动小程序 -->
			<!-- #ifdef MP-TOUTIAO -->
			<view class="product_list" :class="dataStyle">
				<view class="item" v-for="(item, index) in dataList" :key="index" @tap="onGoods(item.id)">
					<view class="img-wrap"><image :src="$wanlshop.oss(item.image, 125, 125)" mode="aspectFill"></image></view>
					<view class="content padding-sm">
						<view class="">
						...
						<view class="flex align-center justify-between">
							...
							<view class="text-sm wanl-gray" v-if="item.comment!==undefined">
								<text>{{item.comment > 9999 ? '9999+':item.comment}} 评价</text>
								<text class="comment margin-left">{{ item.comment > 0 ? parseInt((item.praise / item.comment) * 100) : 0 }}% 好评</text>
							</view>
						</view>
					</view>
				</view>
