		...
		<view class="shop padding-bj solid-bottom bg-white">
			<view class="shopinfo margin-bottom">
				...
				<view class="shopname">
					...
					<view class="wanl-gray text-min" v-if="goodsData.shop.like!==undefined">{{ $wanlshop.toFormat(goodsData.shop.like, 'thousand') }} 人关注店铺</view>
				</view>
				<view class="bnt">
	...
	methods: {
			...
			this.$api.get({
				...
				success: res => {
					...
					if(res.result_msg!=null&&res.result_msg!=""){
						uni.showToast({title:res.result_msg,icon:"none",duration:2500});
					}
				}
			});
		},
		...
		completeSelection() {
			if (this.canCount) {
				...
				if(this.selectNum<this.goodsData.moq){
					this.$wanlshop.msg("起订量最低为"+this.goodsData.moq+"件");
					return false;
				}
				this.ProcessingData();
			}
