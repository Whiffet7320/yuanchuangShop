	...
	<view class="container">
		<view class="category">
			<scroll-view class="scrollbox" :scroll-x="true" :scroll-with-animation="true" :scroll-into-view="scroll_category_index">
				...
				<block v-for="item in common.modulesData.categoryModules" :key="item.id">
		...
		<block v-for="item in common.modulesData.categoryModules" :key="item.id">
		<swiper :current="tabCurrentIndex" style="height: calc(100vh - 88rpx);" duration="300" @change="changeTab">
			<swiper-item class="tab-content" v-for="(categoryData, dindex) in item.childlist" :key="dindex">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					<!-- 空白页 -->
					<wanl-empty :text="empty" v-if="goodsData.length == 0" />
					<wanl-product :dataList="goodsData" :dataStyle="liststyle" v-else />
	...
	export default{
		...
		methods:{
			...
			changeTab(e) {
				...
				this.category_index = this.tabCurrentIndex-1;
				this.category_id = this.category_index<0?0:this.childlist[this.tabCurrentIndex-1].id;
				this.reload = true;
				this.page = 1;
				this.categoryChange(this.category_id,this.category_index);
			},
			//异步加载商品列表
			async loadData() {
				this.$api.get({
		...
		onLoad(option){
			let list = this.common.modulesData.categoryModules;
			list.forEach((item)=>{
