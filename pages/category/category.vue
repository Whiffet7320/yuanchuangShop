<template>
	<view class="category">
		...
		<view class="cu-list grid col-3 no-border" v-if="common.appStyle.category_style == 3">
			<block v-for="(item,index) in common.modulesData.categoryModules" :key="item.id">
				<view class="cu-item" v-for="(categoryData, infokey) in item.childlist" :key="categoryData.id" @tap="productList(categoryData.id, categoryData.name)">
					<image :src="$wanlshop.oss(categoryData.image, 80, 80)" class="list-image" />
					<text class="text-sm">{{ categoryData.name }}</text>
				</view>
			</block>
		</view>
		<view class="wanl-category two" v-if="common.appStyle.category_style == 33">
			...
			<view class="cu-modal bottom-modal" :class="modalName == 'share' ? 'show' : ''" @tap="hideModal">
				<view class="cu-dialog" @tap.stop="">
					<wanl-share 
						...
						:href="common.appConfig.domain+'/pages/category/category?QRtype=category'"
					@change="hideModal"/>
				</view>
			</view>
		</view>
