<template>
	<view class="find" :style="{
		...
		<view class="cu-custom" :style="{ height: wanlsys.height + 'px' }">
			<view class="cu-bar"
				:style="{ height: wanlsys.height + 'px', paddingTop: wanlsys.top + 'px', color: common.appStyle.find_font_color == 'light' ? '#ffffff' : '#333333' }">
				<view class="action">
					<!-- #ifndef MP -->
					<!-- <text class="wlIcon-guanzhu4" style="margin-left: 0;" @tap="$wanlshop.auth('/pages/user/follow')"></text> -->
					<!-- #endif -->
					<!-- #ifdef MP -->
					<text class="wlIcon-zan1" style="margin-left: 0; margin-right: 0;" @tap="$wanlshop.auth('/pages/apps/find/lists')"></text>
				...
				<view class="content" :style="{top: wanlsys.top + 'px'}">
					<text class="text-xl" v-if="wechat">社区种草</text>
					<text class="text-xl" v-else>发现</text>
				</view>
				<!-- #ifndef MP -->
				<!-- <view class="action">
					<text class="wlIcon-zan1" @tap="$wanlshop.auth('/pages/apps/find/lists')"></text>
					<text class="wlIcon-xiaoxizhongxin" @tap="$wanlshop.on('/pages/tabBar/notice/notice')"></text>
					<view class="cu-tag badge" v-if="statistics.notice.chat > 0">{{ statistics.notice.chat }}</view>
				</view> -->
				<!-- #endif -->
			</view>
		...
		<!-- 选项卡 -->
		<scroll-view class="navTab" :style="{color: common.appStyle.find_font_color == 'light' ? '#ffffff' : '#333333'}"
			scroll-x scroll-with-animation :scroll-left="scrollLeft">
			<view class="flex text-center">
				<view class="flex-sub" v-for="(item, index) in tabData" :key="item.id" @tap="tabSelect(index)">
					<view class="text-30" v-if="item.id == 'live'">
						<text class="wlIcon-zhibozhong-0 margin-right-xs"></text> {{item.name}}</view>
		...
		<!-- 主体 -->
		<view class="main">
			<swiper :current="tabCur" class="swiper-box" :style="{height: height + 'px'}" :duration="300"
				@change="onTabchange">
				<swiper-item class="swiper-item" v-for="(item, index) in tabData" :key="item.id">
					<!-- 当前页 -->
					<scroll-view class="swiper-box" :style="{height: height + 'px'}" scroll-y="true"
						refresher-enabled="true" :lower-threshold="50" refresher-background="#f7f7f7"
						:refresher-threshold="100" :refresher-triggered="item.triggered" @scrolltolower="onTolower"
						...
						<view
							...
							v-if="item.id == 'all' && !user.isLogin">
							<view class="text-center">
								<text class="wanl-gray">登录后才能看到您关注店铺的动态哦</text>
								<view class="margin-top-sm">
									<button class="cu-btn bg-orange"
										@tap="$wanlshop.auth('/pages/tabBar/find/find')">马上登陆</button>
								</view>
							</view>
						</view>

						<view class="wanl-find margin-top-sm" v-for="(find, key) in item.data" :key="find.id">
							<view v-if="wxchat" class="margin-lr-bj margin-bottom-bj m bg-white radius-bock padding-lr-bj padding-top-bj">
								...
								<view class="userinfo">
									<view class="avatar">
										<view @tap="onShop(find.shop_id)" class="cu-avatar round margin-right-bj"
											:style="{ backgroundImage: 'url(' + $wanlshop.oss(find.shop.avatar, 35, 35, 2, 'avatar') + ')' }">
										</view>
										<view>
											<view class="wanl-find-head">
												<text @tap="onShop(find.shop_id)" class="wanl-black margin-right-bj shopname">{{find.shop.shopname}}</text>
												<view v-if="find.isLive" @tap="onLive(find.isLive.id)" class="cu-tag sm wanl-bg-orange round">
													<text class="wlIcon-zhibo"></text>直播中</view>
											</view>
											<view class="wanl-gray text-min">{{$wanlshop.timeToDate(find.createtime)}}</view>
										</view>
									</view>
									<block v-if="find.isShopBut">
										<view class="cu-btn round line-red sm" @tap="onShop(find.shop_id)"> <text
												class="wlIcon-dianpu1"></text> 进店 </view>
									</block>
									<block v-else>
										<view class="cu-tag round bg-white" v-if="find.isFollow"
											@tap="onFollow(index, key)">
								...
								<view class="container radius-bock">
									<block v-for="(img, kes) in find.images" :key="kes">
										<block v-if="find.type == 'live'">
											<view class="item" @tap="onFind(find)" :class="[kes == 0 ? 'item-live':'']"
												:style="{ backgroundImage: 'url(' + $wanlshop.oss(img, 200, 0) + ')' }">
												<block v-if="kes == 0">
													<view class="play">
														<text
															:class="[find.live.state == 1?'wlIcon-icon_zhibo-mian':'wlIcon-guijihuifang']"></text>
													</view>
													<view class="state text-white" v-if="find.live.state == 1">
														<view class="tags wanl-bg-orange"><text
																class="wlIcon-zhibo"></text> <text
																class="text-min">直播</text></view>
														<view class="text-sm">{{find.live.views}} 在看</view>
													</view>
													<view class="state text-white" v-else>
														<view class="tags bg-grey"><text class="text-min">回放</text>
														</view>
													...
													<view class="like text-center text-white"
										...
										<block v-else>
											<block v-if="find.images.length == 1">
												<view class="item item-1" @tap="previewImage(find.images, kes)"
													:style="{ backgroundImage: 'url(' + $wanlshop.oss(img, 200, 0) + ')' }">
												</view>
											</block>
											<block v-else-if="find.images.length == 3">
												<view class="item" @tap="previewImage(find.images, kes)"
													:class="[kes == 0 ? 'item-3':'']"
													:style="{ backgroundImage: 'url(' + $wanlshop.oss(img, 200, 0) + ')' }">
												</view>
											</block>
								...
								<scroll-view class="scroll-view margin-top-sm" scroll-x="true">
									<view class="scroll-view-item radius-bock"
										...
										v-for="(goods, sub) in find.goods" :key="goods.id" @tap="onGoods(goods.id)">
										<view class="img">
										...
										<view class="icon text-lg wanl-gray-light"><text class="wlIcon-fanhui6"></text>
										</view>
									</view>
								</scroll-view>
								<view class="flex justify-between padding-tb-bj align-center">
									<view class="wanl-gray-light text-sm" v-if="find.type == 'live'"
										@tap="onFind(find)">{{find.live.views}} 阅读</view>
									<view class="wanl-gray-light text-sm" v-else @tap="onFind(find)">{{find.views}} 阅读
									</view>
									<view class="flex">
										...
										<view class="margin-left" @tap="onFind(find)"
								...
								<view class="padding-tb-bj solid-top text-center text-orange" v-if="find.newGoods > 0"
									@tap="onShop(find.shop_id)">
									进店查看 {{find.newGoods}} 件新品
									<text class="wlIcon-fanhui6 margin-left-xs"></text>
								</view>
							</view>
							<view v-else class="margin-lr-bj margin-bottom-bj m bg-white radius-bock padding-lr-bj padding-top-bj">
								<view class="margin-bottom-sm wanl-gray-light text-min flex justify-between" v-if="item.id == 'all' && !find.isShopBut">
									<text>你可能喜欢的内容</text>
									<text class="wlIcon-fanhui4"></text>
								</view>
								<image mode="aspectFill" src="https://scht.gycsc.com/uploads/20210902/30e426cf36e4cf2178da5700669a3283.jpg" style="width: 100%;height:400rpx;"></image>
							</view>
						</view>
