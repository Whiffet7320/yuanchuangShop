<template>
	<view>
		<!-- #ifdef MP -->
		<view class="cu-custom text-white"
			:style="{color: common.appStyle.user_font_color == 'light'?'#ffffff':'#222222'}">
			<view class="cu-bar fixed" :style="{ height: wanlsys.height + 'px', paddingTop: wanlsys.top + 'px' }">
				<view class="action mp text-bold">
					<text class="wlIcon-shezhi" style="margin-right: 0.6em;"
				...
				<view class="text-lg" @tap="$wanlshop.auth('/pages/user/setting/user')">
					<view v-if="headerOpacity == 1">
						<view class="cu-avatar round margin-right-xs"
							:style="{ backgroundImage: 'url(' + $wanlshop.oss(user.avatar, 35, 35, 2, 'avatar') + ')' }">
						</view>
		...
		<view class="wanl-user" :style="{ 
			...
			<view class="user" :style="{ paddingTop: wanlsys.height + 'px' }">
				<view class="avatar margin-right-bj" @tap="portrai">
					<image :src="$wanlshop.oss(user.avatar, 62, 62, 2, 'avatar')" mode="widthFix"></image>
				</view>
				...
				<view class="content" @tap="$wanlshop.auth('/pages/tabBar/user/user')" v-else>
			...
			<view class="operate">
				...
				<view class="text-min" @tap="$wanlshop.auth('/pages/user/order/order')">
					<text
						class="text-bold">{{ $wanlshop.toFormat(statistics.order.pay+statistics.order.delive+statistics.order.receiving+statistics.order.evaluate, 'hundred') }}</text>
					全部订单
				</view>
				...
				<view class="bg-white radius grid text-center col-2 padding-lr-bj padding-tb-sm">
					<view class="solid-right flex justify-between" @tap="$wanlshop.auth('/pages/user/coupon/list')">
						...
						<view class="cu-avatar"
							:style="{ backgroundImage: 'url(' + $wanlshop.appstc('/user/icon_card_bag.png') + ')' }">
						</view>
					</view>
					<view class="flex justify-between" @tap="$wanlshop.auth('/pages/user/signin/log')">
						...
						<view class="cu-avatar"
							:style="{ backgroundImage: 'url(' + $wanlshop.appstc('/user/icon_super_vip.png') + ')' }">
						</view>
					</view>
				</view>
			</view>
		...
		<view class="nav1">
			<view class="tit1">
				<view class="box1" @click="toWodefabu">
					<image class="pic1" src="/static/images/img/zu1806.png" mode=""></image>
					<view class="txt1">我的发布</view>
				</view>
				<view class="box1" @click="toRenwudingdan">
					<image class="pic1" src="/static/images/img/zu1808.png" mode=""></image>
					<view class="txt1">任务订单</view>
				</view>
			</view>
			<view class="tit1">
				<view class="box1" @click="toWodejiedan">
					<image class="pic1" src="/static/images/img/zu1807.png" mode=""></image>
					<view class="txt1">我的接单</view>
				</view>
				<view class="box1" @click="toJiedandingdan">
					<image class="pic1" src="/static/images/img/zu1810.png" mode=""></image>
					<view class="txt1">接单订单</view>
				</view>
			</view>
		</view>
		...
		<view class="wanl-user-tool padding-top-bj margin-lr-bj">
			<view class="list text-min grid col-5 wanl-gray-dark">
				...
				<view>
					<view @click="toWodetuiguan" style="display: flex;flex-direction: column;align-items: center;">
						<image src="/static/images/img/tubb.png"
							style="width: 46rpx;height: 46rpx;margin-top: 6rpx;margin-bottom: 14rpx;" mode=""></image>
						我的推广
					</view>
				...
				<view @tap="$wanlshop.auth('/pages/user/setting/setting')">
					<text class="wlIcon-shezhi1 wanl-text-green"></text>
					设置
				</view>
				<view @tap="$wanlshop.auth('/pages/External/WebView')">
					<text class="wlIcon-backstage"></text>
					商家后台
				</view>
			</view>
		</view>
		<view class="wanl-you-like"
			:style="{ backgroundImage: 'url(' + $wanlshop.appstc('/common/guess_you_like_it.png') + ')' }"></view>
...
<script>
	...
	export default {
		...
		computed: {
			...mapState(['user', 'statistics', 'common','myinvite_code'])
		},
		onPullDownRefresh() {
			this.loadData();
		},
		...
		onLoad(options) {
			...
			if (this.invite_code) {
				if (uni.getStorageSync("wanlshop:user").token) {
					uni.showModal({
						...
						success: (res) => {
							if (res.confirm) {
								console.log('用户点击确定');
								uni.request({
									url: `${this.$url}/wanlshop/Fenxiao/invite_bind`,
									method: 'POST',
									header: {
									...
									success: ress => {
										console.log(ress)
										this.$wanlshop.msg(ress.data.msg);
									},
								})
							}
						}
					...
					uni.navigateTo({
						url: `/pages/user/auth/register?invite_code=${this.invite_code}`
					})
				}
			}
