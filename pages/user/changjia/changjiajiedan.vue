		...
		<swiper :current="tabCurrentIndex" style="height: calc(100% - 80rpx)" duration="300" @change="changeTab">
			<swiper-item class="tab-content" v-for="(tag, key) in navList" :key="tag.state">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					<!-- 空白页 -->
					<wanl-empty text="没有找到任何订单" v-if="tag.orderList.length === 0" />
					<!-- 任务列表 -->
					<view class="items">
						<view class="list" @click="toRenwuxiangqin(item)" v-for="(item,i) in tag.orderList" :key='item.id'>
							<view class="hd flex">
							...
							<view class="ft flex">
								...
								<view class="right flex">
									<text style="color: #333333;">{{item.issuer_name}}（{{item.issuer_mobile}}）</text>
								</view>
							</view>
							<view class="myBtn">联系采购商</view>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			toRenwuxiangqin(item){
				uni.navigateTo({
					url:`/pages/user/changjia/dingdanxiangqin?id=${item.id}`
				})
			},
			//获取订单列表
			loadData(source) {
