		...
		<swiper :current="tabCurrentIndex" style="height: calc(100% - 80rpx)" duration="300" @change="changeTab">
			<swiper-item class="tab-content" v-for="(tag, key) in navList" :key="tag.state">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					<!-- 空白页 -->
					<wanl-empty text="没有找到任何任务" v-if="tag.orderList.length === 0" />
					<!-- 任务列表 -->
					<view class="items">
						<view class="list" @click="torenwuxiangqin(item)" v-for="(item,i) in tag.orderList" :key='item.id'>
							...
							<view class="bd flex">
								...
								<view v-if="item.status == 0" class="right">未达成合作</view>
								<view v-if="item.status == 1" class="right">待采购商确认</view>
								<view v-if="item.status == 2" class="right">{{item.kehu_name}}</view>
								<view v-if="item.status == 3" class="right">{{item.kehu_name}}</view>
							</view>
							<view class="ft flex">
