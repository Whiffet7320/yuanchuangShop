	...
	<view class="index">
		...
		<view class="nav7 nav3 nav2" v-if="obj.renwu.trade_status >= 3">
			...
			<view class="cu-item" @click="towuliu">
				...
				<view class="content">
					<view>
						<text v-if="obj.kd.length>0" class="text-cut-2 text-sm wanl-pip" style="font-size: 24rpx;line-height: 26rpx;">{{obj.kd[0].context}}</text>
						<text v-else class="text-cut-2 text-sm wanl-pip" style="font-size: 24rpx;line-height: 26rpx;">暂无物流信息</text>
					</view>
		...
		<view class="nav7 nav3 nav2" v-if="obj.renwu.trade_status < 2">
		...
		<view class="nav7 nav3 nav2" v-else>
			<view class="tit2">收货信息</view>
			<view class="address cu-list menu-avatar margin-bj radius-bock">
				<view class="cu-item">
					<view class="cu-avatar round wanl-bg-orange"><text class="wlIcon-weizhi1"></text></view>
					<view class="content" v-if="addressData.address">
						<view>
							<text class="wanl-pip margin-right-sm">{{ addressData.name }}</text>
							<text class="wanl-gray-light text-sm">{{ addressData.mobile }}</text>
						</view>
		...
		<view class="nav4 nav3 nav2" v-if="obj.renwu">
			...
			<view class="nav-3">
				...
				<view class="tit-3">
					...
					<view v-if="obj.renwu.fktype == 2" class="txt-3-2">
						{{obj.renwu.fktime == '' ? '未付款':obj.renwu.fktime}}(预付款)
					</view>
					<view v-if="obj.renwu.fktype == 1" class="txt-3-2">
						{{obj.renwu.fktime == '' ? '未付款':obj.renwu.fktime}}(全款)
					</view>
				</view>
		...
		<!-- 未付款 -->
		<view class="footer" v-if="obj.renwu.trade_status==0">
			<view class="btn1">联系厂家</view>
			<view @click="toFukuan" class='btn2 isHaveDD'>去付款</view>
		</view>
		<!-- 待发货/已完成 -->
		<view class="footer" v-if="obj.renwu.trade_status==2 || obj.renwu.trade_status==4">
			<view class="btn3">联系厂家</view>
		</view>
		<!-- 待收货 -->
		<view class="footer" v-if="obj.renwu.trade_status==3">
			<view class="btn1">联系厂家</view>
			<view @click="querenshouhuo" class='btn2 isHaveDD'>确认收货</view>
		</view>
		<!-- 已取消 -->
		<view class="footer" v-if="obj.renwu.trade_status==1">
...
<script>
	...
	export default {
		...
		methods: {
			querenshouhuo() {
				uni.request({
					url: `${this.$url}/wanlshop/Jiedan/confirm_order`,
					data: {
						id: this.id,
					},
					method: 'POST',
			...
			seeXiangqin() {
				uni.navigateTo({
					url: `/pages/user/caigoushang/renwuxiangqin?id=${this.id}`
				})
			},
			towuliu(){
				uni.navigateTo({
			...
			toFukuan() {
				...
				uni.navigateTo({
					url: `/pages/user/caigoushang/shouyintai1?money=${money}&id=${this.id}&type=${type}`
				})
			},
			addAddress(type) {
