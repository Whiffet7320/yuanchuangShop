...
export default {
	...
	methods: {
		...
		formSubmit: function(e) {
			...
			if (checkRes) {
				this.$api.get({
					...
					success: res => {
						this.$wanlshop.to(`validcode?event=register&mobile=${e.detail.value.mobile}&url=${this.pageroute}&invite_code=${this.invite_code}`,'slide-in-bottom',200);
					},
					fail: res => {
						uni.showModal({
						    title: '手机号已被注册',
		...
		auth() {
			if(this.invite_code != ''){
				uni.navigateTo({
					url:`/pages/user/auth/auth?invite_code=${this.invite_code}`
				})
			}else{
