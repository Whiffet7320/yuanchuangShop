<template>
	<view class="auth-wrap">
		<view class="auth">
			...
			<form @submit="formSubmit">
				<view class="auth-group radius-bock bg-gray wlian-grey-light">
					<input placeholder="请输入手机号 / 用户名" placeholder-class="placeholder" name="account" type="text"
						maxlength="16" confirm-type="next" @input="onKeyInput"></input>
				</view>
				<view class="auth-button flex flex-direction">
					<button form-type="submit" class="cu-btn sl radius-bock bg-orange"
						:disabled="submitDisabled">下一步</button>
					<!-- #ifdef MP-WEIXIN -->
					<view class="wanl-weixin-btn-info margin-tb-bj text-center text-sm">或</view>
					<button type="primary" class="cu-btn sl radius-bock bg-no" open-type="getPhoneNumber"
						@getphonenumber="decryptPhoneNumber">获取手机号一键登录</button>
					<!-- #endif -->
				</view>
			</form>
		</view>
		<view class="auth-foot">
			...
			<view class="oauth" v-if="wxchat">
				<view class="cu-avatar lg round bg-white wlIcon-WeChat" @tap="tologins()"></view>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			console.log(option,this.invite_code)
						...
						this.$api.post({
							...
							success: res => {
								...
								if (res.binding == 0) {
									...
									console.log(this.invite_code, 1111111)
									if (this.invite_code) {
										uni.switchTab({
											url: `/pages/tabBar/user/user?invite_code=${this.invite_code}`
										})
									} else {
										uni.reLaunch({
			...
			formSubmit(e) {
				...
				if (checkRes) {
							...
							uni.showModal({
								...
								success: (res) => {
									if (res.confirm) {
										if (this.invite_code) {
											uni.navigateTo({
												url: `/pages/user/auth/name?invite_code=${this.invite_code}&mobile=${formData.account}`
											})
										} else {
											this.$wanlshop.to(
					...
					if (formData.account) {
						if (this.invite_code) {
							uni.navigateTo({
								url: `/pages/user/auth/name?invite_code=${this.invite_code}&name=${formData.account}`
							})
						} else {
